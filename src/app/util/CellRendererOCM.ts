		...
		if (params.node.footer) {
			switch (params.node.level) {
				...
				case 0: // Total primer nivel.
					return `<h1 style="text-align: right; color: red; font-size: 11px; font-weight: bold">${valorFormateado}</h1>`;
			...
			return `<h1 style="font-size: 11px; text-align: right">${valorFormateado}</h1>`;
		}
	} else {
		return '';
	}
