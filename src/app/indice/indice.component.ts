...
import contratosYlicitacionesJSON from '../../assets/data/todo052023NoRepeatOkCIFOK.json';
...
export class IndiceComponent implements AfterViewInit, OnInit {
	...
	constructor(
		...
		private _channelFilterDateService: ChannelFilterDateService
	) {
	...
	changeProcedure(item: IProcedureChecked, checked: boolean) {
		...
		if (
			listCheckedFalse.length > 0 &&
			listCheckedFalse.length === this.typeProcedureListChecked.length
		) {
	...
	changeType(item: IProcedureChecked, checked: boolean) {
		...
		if (
			listCheckedFalse.length > 0 &&
			listCheckedFalse.length === this.typeCodeListChecked.length
		) {
	...
	private filterRange(data: ILicitacion[]): ILicitacion[] {
		if (this.rangeEnd > 0 && this.rangeInit > 0) {
			data = data.filter(
				(item) =>
					item.TaxExclusiveAmount >= this.rangeInit && item.TaxExclusiveAmount <= this.rangeEnd
			);
